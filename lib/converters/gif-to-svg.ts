/**
 * GIF to SVG Converter Implementation
 * 
 * This module provides GIF to SVG conversion using:
 * 1. imagetracerjs for direct client-side conversion (primary method)
 * 2. CloudConvert GIF → PNG → client-side PNG to SVG (fallback)
 */
...
/**
 * Extended conversion options for GIF to SVG
 */
interface GifToSvgOptions extends ConversionOptions, ImageTracerOptions {
  /** Quality level (1-100, default: 50) */
  quality?: number
  /** Maximum image size in pixels (default: 16000000 = 16MP) */
  maxPixels?: number
  /** Frame index to extract from animated GIF (default: 0) */
  frameIndex?: number
}

/**
 * GIF to SVG Converter using imagetracerjs (works in browser)
 */
class GifToSvgConverter extends LazyLoadedConverter {
  name = 'GIF to SVG'
  from: ImageFormat = 'gif'
  to: ImageFormat = 'svg'
  description = 'Convert GIF images to scalable vector graphics by extracting and tracing the first frame'
  ...
  private async convertViaCloudConvert(
    input: Buffer,
    options: GifToSvgOptions
  ): Promise<ConversionResult> {
    try {
      // Step 1: Convert GIF to PNG using CloudConvert
      ...
      if (!pngResult.success) {
        throw new ConversionError(
          `CloudConvert GIF to PNG failed: ${pngResult.error}`,
          'CLOUDCONVERT_GIF_TO_PNG_FAILED'
        )
      }
      ...
      if (!pngResult.data) {
        throw new ConversionError(
          'CloudConvert GIF to PNG returned no data',
      ...
      throw new ConversionError(
        `CloudConvert GIF to SVG chain failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
  ...
  protected async performConversionWithLibraries(
    ...
    try {
      // Validate input is GIF
      const detectedFormat = detectFileTypeFromBuffer(input)
      if (detectedFormat !== 'gif') {
        throw new FileValidationError(
          `Expected GIF file but detected ${detectedFormat || 'unknown'} format`
        )
      }
      ...
      try {
        ...
        console.warn('Direct GIF to SVG conversion failed, falling back to CloudConvert:', directError)
      ...
      throw new ConversionError(
        `GIF to SVG conversion failed: ${message}`,
        'GIF_TO_SVG_FAILED'
      )
    }
  }
...
export const gifToSvgConverter = new GifToSvgConverter()

// Export the handler for direct use
export const gifToSvgHandler = gifToSvgConverter.handler

// Client-side wrapper
export async function convertGifToSvgClient(
  file: File,
  options: GifToSvgOptions = {}
): Promise<ConversionResult> {
  const arrayBuffer = await file.arrayBuffer()
  const buffer = Buffer.from(arrayBuffer)
  return gifToSvgHandler(buffer, options)
}

// Server-side wrapper
export async function convertGifToSvgServer(
  buffer: Buffer,
  options: GifToSvgOptions = {}
): Promise<ConversionResult> {
  return gifToSvgHandler(buffer, options)
