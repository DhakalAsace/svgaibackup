...
interface JpgToSvgOptions extends ConverterOptions {
  threshold?: number
  color?: boolean
  colorLevels?: number
  optimization?: number
...
export default function JpgToSvgConverter() {
  const [options, setOptions] = useState<JpgToSvgOptions>({
    threshold: 128,
    color: false,
    colorLevels: 4,
    optimization: 5,
  ...
  const handleConvert = async (file: File, converterOptions: ConverterOptions): Promise<ConversionResult> => {
    try {
      ...
      const result = await jpgToSvgHandler(buffer, {
        ...mergedOptions,
        onProgress: (progress) => {
          // Progress is handled by ConverterUI
          console.log(`Conversion progress: ${(progress * 100).toFixed(0)}%`)
        }
      })
