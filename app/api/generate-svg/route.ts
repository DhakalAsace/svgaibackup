          ...
          if (isSubscribed) {
            ...
            if (profile) {
              await supabaseAdmin
                .from('profiles')
                .update({ 
                  monthly_credits_used: Math.max(0, profile.monthly_credits_used - 2)
                })
                .eq('id', user.id);
            }
          } else {
            ...
            if (profile) {
              await supabaseAdmin
                .from('profiles')
                .update({ 
                  lifetime_credits_used: Math.max(0, profile.lifetime_credits_used - 2)
                })
                .eq('id', user.id);
            }
          }
